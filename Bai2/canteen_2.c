...
void waiter(int shmid, int semid)
{
    ...
    while (data->served < data->k)
    {
        ...
        if(data->holding_p + data->holding_c + data->holding_d >= data->k*2+1) break;
        
        while (data->p - data->served - data->holding_p > 0 
            || data->c - data->served - data->holding_c > 0 
            || data->d - data->served - data->holding_d > 0)
        {
...
int main(int argc, char *argv[])
{
    ...
    for(int i = 1; i <= K; i++)
      if (fork() == 0) {
        customer(shmid, semid, i);
        exit(0);
      }
    // Chờ tất cả tiến trình con kết thúc
    for (int i = 0; i < 2+K; i++) wait(NULL);
    
    // Giải phóng tài nguyên
    shmctl(shmid, IPC_RMID, NULL);
    semctl(semid, 0, IPC_RMID, 0);

    end = time(NULL); // Lấy thời gian kết thúc
    double cooking_time = difftime(end, start); // Tính thời gian nấu ăn
